...
class AronaConfig(
    ...
    companion object {
        //所有托管的配置表
        val aronaConfigMap: HashMap<AronaPlugin, AronaConfig> = hashMapOf()
    }
    ...
    init {
        //是否从jar写出资源
        if (saveResource) paimonPlugin.saveResource(configName, false)
        ...
        watchTask.setWatcher(object : WatcherImpl() {
            override fun onModify(p0: WatchEvent<*>?, p1: Path?) {
                this@AronaConfig.reloadConfig()
                this@AronaConfig.configReload?.invoke(this@AronaConfig)
            }
        })
        //启动!
        watchTask.start()
    ...
    var configReload: ((AronaConfig) -> Unit)? = null

    /**
     * 当资源文件重新加载时触发该事件，让开发者知道资源文件已加载
     * @param e 事件回调参数
     */
    fun configReload(e: ((AronaConfig) -> Unit)) {
        this.configReload = e
    }
}